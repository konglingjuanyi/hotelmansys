...
<html>
  <head>
   <head>
		...
		<script type="text/javascript">
			/**弹出页面处理**/
			function toWindow(type,room_id){
				var path = art.dialog.data('homeDemoPath') || './';// 
				switch(type){
					case 'mod':
						art.dialog.open('modRoomRoomAction.action?room.room_id='+room_id, {
							id: 'room_mod_win',
							title: '修改房间界面',
							width:600,
							height:500,
						...
						case 'add':
							art.dialog.open('addRoomRoomAction.action', {
								id: 'room_add_win',
								title: '添加类型界面',
								width:600,
								height:500,
						...
						case 'del':
							...
							if(result==true){
								art.dialog.open('delRoomRoomAction.action?room.room_id='+room_id, {
										id: 'room_del_win',
										title: '删除房间界面',
										width:500,
										height:300,
	...
	<body>
		...
		<div class="div_bk">
			...
			<!-- 搜索区 -->
			<form action="findRoomAction.action" method="post" name="roomQueryForm">
				<!-- 赋值隐藏域 -->
				<!-- 赋值当前页数 【模型驱动】-->
				...
				<table class="maintable">
	   				<tr>
	   					<th>客房名称</th>
	   					<td colspan="3" ><input type="text" class="text" name="query.roomName" value="<s:property value="query.roomName"/>"/></td>
	   					<th>房间号</th>
	   					<td colspan="3" ><input type="text" class="text" name="query.roomSite" value="<s:property value="query.roomSite"/>"/></td>
	   				</tr>
	   				<tr align="center">
	   					<td colspan="4">&nbsp;</td>
   			...
   			<table class="listtable stripe">
   				<tr>
   					<th width="120">序号</th>
   					<th width="120">房间名称</th>
   					<th width="120">房间号</th>
   					<th width="120">房间容量</th>
   					<th width="120">房间单价</th>
   					<th width="150">是否特价</th>
   					<th width="120">优惠价格</th>
   					<th width="120">房间状态</th>
   					<th width="160">操作</th>
   				</tr>
   				<s:iterator value="pageBean.datas" var="room" status="st">
   					<tr>
	   					...
	   					<td align="center" >
	   						<s:if test="#room.room_bargain==\"1\"">
	   							<strong><font color="green">正常</font></strong>
	   						</s:if>
	   						<s:else>
	   							<strong><font color="red">特价</font></strong>
	   							<strong><font color="#EE1289"><s:date name="#room.bar_startDate" format="yyyy-MM-dd"/>&nbsp;<s:date name="#room.bar_endDate" format="yyyy-MM-dd"/></font></strong>
	   						</s:else>
	   					</td>
	   					<td align="center"><s:property value="#room.room_barprice"/></td>
	   					<td align="center" >
	   						<s:if test="#room.room_status==\"1\"">
	   					...
	   					<td align="center">
		   					<a href="javascript:void(0)" onclick="toWindow('mod','<s:property value="#room.room_id"/>')" class="edit">编辑</a>
		   					<a href="javascript:void(0)" onclick="toWindow('del','<s:property value="#room.room_id"/>')" class="delete">删除</a>
	   					</td>
   					</tr>
   				</s:iterator>
   			</table>
