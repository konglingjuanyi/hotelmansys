...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		function toJSONAjax(){
  			...
  			var product_id = '<s:property value="%{#product.pro_id}" />';
  			//JSON不能使用模型驱动
  			//解决办法
  			var params = "product_name="+product_name+"&product.pro_id="+product_id;
  			$.ajax({
  				type:"post",
  				url :"modProductJsonJsonAjaxAction.action",
  				data:params,
  				dataType:"json",
  ...
  <body>
    <form action="toModProductAction.action?product.pro_id=<s:property value="#product.pro_id" />" method="post" onsubmit="return validate()">
    	<center>
	    	<table cellpadding="0" cellspacing="0" width="90%">
	    		<thead>
	    			<tr align="center">
	    				<td colspan="3"><font color="green" size="4px"><b>修改商品</b></font></td>
	    			</tr>
	    		</thead>
	    		<tbody>
	    			<tr>
	    				<td>商品名称:</td>
	    				<td><input value='<s:property value="#product.pro_name" />' type="text" name="product.pro_name" id="pro_name" onblur="nameBlur();" /></td>
	    				<td width="150px" id="pro_name_id">&nbsp;</td>
	    			</tr>
	    			<tr>
	    				<td>商品类型:</td>
	    				<td colspan="2">
	    					<s:select value='%{#product.pro_typeId}' list="#typeList" listKey="type_id" listValue="type_name" name="productType_id" ></s:select>
	    				</td>
	    			</tr>
	    			<tr>
	    				<td>商品进价:</td>
	    				<td><input value='<s:property value="#product.pro_inprice" />' type="text" name="product.pro_inprice" id="pro_inprice" onblur="inpriceBlur()" /></td>
	    				<td width="150px" id="pro_inprice_id">&nbsp;</td>
	    			</tr>
	    			<tr>
	    				<td>商品售价:</td>
	    				<td><input value='<s:property value="#product.pro_price" />' type="text" name="product.pro_price" id="pro_price" onblur="priceBlur()" /></td>
	    				<td width="150px" id="pro_price_id">&nbsp;</td>
	    			</tr>
	    			<tr>
	    				<td>商品库存:</td>
	    				<td>
	    					<input value='<s:property value="#product.pro_store" />' type="text" name="product.pro_store" id="pro_store" onblur="storeBlur()" />
	    			...
	    			<tr>
	    				<td>生产日期:</td>
	    				<td>
	    					<input value='<s:property value="#product.pro_startDate" />' id="pro_startDate" name="product.pro_startDate" class="Wdate" type="text" onfocus="WdatePicker({skin:'whyGreen',maxDate:'%y-%M-%d'})"/>
	    				</td>
	    			...
	    			<tr>
	    				<td>过期日期:</td>
	    				<td>
	    					<input value='<s:property value="#product.pro_endDate" />' name="product.pro_endDate" id="pro_endDate" class="Wdate" type="text" onfocus="WdatePicker({minDate:'%y-%M-{%d+1}'})"/>
	    				</td>
